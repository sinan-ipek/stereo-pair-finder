...
data class PairCandidate(val index: Int, val left: Photo, val right: Photo) {
    val seconds: Double? = left.takenAtMillis?.let { a -> right.takenAtMillis?.let { b -> kotlin.math.abs(b-a)/1000.0 } }
}
enum class PairStatus(val text: String) {
    MATCHED("Eşleşti"), TIME_MISSING("Çekim zamanı bulunamadı"), TIME_EXCEEDED("Zaman sınırı aşıldı"),
    SIMILARITY_LOW("Benzerlik eşiğinin altında"), EVIDENCE_LOW("Benzerlik güveni yetersiz"),
    ALIGNMENT_FAILED("Hizalama başarısız"), ALIGNMENT_LOW("Düşük hizalama güveni"), AREA_LOW("Ortak görüntü alanı yetersiz")
}
...
object PairPolicy {
    ...
    fun adjacent(photos: List<Photo>) = sorted(photos).zipWithNext().mapIndexed { i, (a,b) -> PairCandidate(i+1,a,b) }
    fun status(pair: PairCandidate, similarity: Double, evidence: Boolean, aligned: Boolean, confidence: Double, area: Double,
               maxSeconds: Int = DEFAULT_MAX_SECONDS, threshold: Int = DEFAULT_SIMILARITY): PairStatus = when {
