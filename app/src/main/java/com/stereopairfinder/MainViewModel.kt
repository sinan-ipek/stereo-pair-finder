...
class MainViewModel(app: Application):AndroidViewModel(app) {
    ...
    fun analyze(){ if(_state.value.selected.size<2){_state.value=_state.value.copy(message="En az iki fotoğraf seçin.");return}; work?.cancel(); work=viewModelScope.launch(Dispatchers.Default){
        val snapshot=_state.value; val pairs=PairPolicy.adjacent(snapshot.selected); val results=mutableListOf<AnalysisResult>(); val analyzer=StereoAnalyzer()
        ...
        try { pairs.forEachIndexed { i,pair -> ensureActive(); _state.value=_state.value.copy(stage="Çift ${i+1}/${pairs.size} hizalanıyor")
            val l=reader.bitmap(pair.left.uri,FINAL_SOURCE_MAX_SIDE); val r=reader.bitmap(pair.right.uri,FINAL_SOURCE_MAX_SIDE)
            val result=try { analyzer.analyze(pair,l,r,snapshot.maxSeconds,snapshot.similarity) } finally {
                if(!l.isRecycled) l.recycle(); if(!r.isRecycled) r.recycle()
            }
            results+=result
    ...
    fun save(result:AnalysisResult){val jpeg=result.sbsJpeg?:return; viewModelScope.launch(Dispatchers.IO){runCatching{saver.saveJpeg(jpeg)}.onSuccess{_state.value=_state.value.copy(message="${it.name}, ${it.location} konumuna kaydedildi")}.onFailure{_state.value=_state.value.copy(message="Kayıt başarısız: ${it.message}")}}}
    companion object { private const val FINAL_SOURCE_MAX_SIDE = 4096 }
}
