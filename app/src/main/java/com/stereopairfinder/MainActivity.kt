...
private fun ResultCard(result: AnalysisResult, onSave: (AnalysisResult) -> Unit) {
    ...
    Card {
        Column(
            ...
            Text("Uygulanan düşey translation: %.2f px".format(result.verticalTranslationPx))
            Text("Translation sonrası düşey ortak alan: %.1f%%".format(result.commonAreaRatio * 100))
